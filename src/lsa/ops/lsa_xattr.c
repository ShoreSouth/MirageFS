                       ...
                       size_t *actual)
{
    ...
    FS_LOG_DUMP_INFO("enter: fd=%d, name=%s, size=%zu", fd,
                     name ? name : "(null)", size);
    ...
    if (ret < 0)
    {
        ...
        FS_LOG_DUMP_ERROR("getxattr failed: fd=%d, name=%s, err=%s (0x%x)", fd,
...
lsa_ret_t lsa_setxattr(int fd, const char *name, const void *value, size_t size,
                       fs_flags_t flags)
{
    int xattr_flags = 0;
    lsa_ret_t err;

    FS_LOG_DUMP_INFO("enter: fd=%d, name=%s, size=%zu, flags=0x%x", fd,
                     name ? name : "(null)", size, flags);
    ...
    if (fs_flag_test(flags, FS_FLAG_EXCLUSIVE))
    {
        xattr_flags |= XATTR_CREATE;
    }

    if (fsetxattr(fd, name, value, size, xattr_flags) < 0)
    {
        err = lsa_error(FS_OP_SETXATTR, errno);
        FS_LOG_DUMP_ERROR("setxattr failed: fd=%d, name=%s, err=%s (0x%x)", fd,
                          name, fs_error_str(err), err);
        return err;
    }
...
lsa_ret_t lsa_listxattr(int fd, char *list, size_t size, size_t *actual)
{
    ...
    if (ret < 0)
    {
        ...
        FS_LOG_DUMP_ERROR("listxattr failed: fd=%d, err=%s (0x%x)", fd,
                          fs_error_str(err), err);
        return err;
    }
...
lsa_ret_t lsa_removexattr(int fd, const char *name)
{
    ...
    FS_LOG_DUMP_INFO("enter: fd=%d, name=%s", fd, name ? name : "(null)");
