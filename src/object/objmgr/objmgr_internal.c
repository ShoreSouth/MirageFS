...
static uint64_t objmgr_handle_hash(const obj_handle_t *handle)
{
    uint64_t hash;
    uint16_t i;
...
static bool objmgr_handle_equal(const obj_handle_t *lhs,
                                const obj_handle_t *rhs)
{
    ...
    if ((lhs->mount_id != rhs->mount_id) || (lhs->type != rhs->type) ||
        (lhs->len != rhs->len))
    {
        return false;
    }
...
static uint64_t objmgr_handle_node_hash(const fs_list_head_t *node)
{
...
static uint64_t objmgr_handle_key_hash(const void *key)
{
    return objmgr_handle_hash((const obj_handle_t *)key);
}

static bool objmgr_handle_match(const fs_list_head_t *node, const void *key)
{
    const objmgr_handle_entry_t *entry;
...
static objmgr_handle_entry_t *
objmgr_find_handle_entry(fs_hash_t *table, const obj_handle_t *handle)
{
    fs_list_head_t *node;
...
{
    return fs_hash_init(table, bucket_nr, objmgr_handle_node_hash,
                        objmgr_handle_key_hash, objmgr_handle_match);
}
...
fs_error_t objmgr_insert_handle_locked(obj_runtime_t *rt)
{
    ...
    if (entry == NULL)
    {
        ...
        FS_LOG_DUMP_ERROR("calloc failed, err=%s (0x%x)", fs_error_str(err),
                          err);
        return err;
    }
...
void objmgr_remove_handle_locked(obj_runtime_t *rt)
{
    ...
    entry = objmgr_find_handle_entry(&g_objmgr.handle_table, &rt->meta.handle);
    if (entry == NULL)
    {
        FS_LOG_DUMP_INFO("exit: not found");
        return;
...
obj_runtime_t *objmgr_lookup_locked(const obj_key_t *key)
{
    ...
    rt = objtable_lookup(&g_objmgr.table, key);
...
fs_error_t objmgr_insert_locked(obj_runtime_t *rt)
{
    ...
    ret = objtable_insert(&g_objmgr.table, rt);

    if (fs_failed(ret))
    {
        FS_LOG_DUMP_INFO("exit: failed, err=%s (0x%x)", fs_error_str(ret), ret);
        return ret;
    }
    ...
    fs_atomic32_inc(&g_objmgr.object_count);
...
fs_error_t objmgr_remove_locked(const obj_key_t *key)
{
    ...
    ret = objtable_remove(&g_objmgr.table, key);
    ...
    fs_atomic32_dec(&g_objmgr.object_count);
...
bool objmgr_state_can_transit(obj_state_t from, obj_state_t to)
{
    ...
    FS_LOG_DUMP_INFO("enter: from=%u, to=%u", (unsigned int)from,
                     (unsigned int)to);
    ...
    FS_LOG_DUMP_INFO("exit: %s", can ? "true" : "false");
    return can;
}
...
fs_error_t objmgr_change_state(obj_runtime_t *rt, obj_state_t state)
{
    ...
    FS_LOG_DUMP_INFO("enter: rt=%p, state=%u", (void *)rt, (unsigned int)state);

    if (rt == NULL)
    {
        err = obj_error(OBJ_SUB_STATE, EINVAL);
    ...
    if (!objmgr_state_can_transit(objruntime_state(rt), state))
    {
        err = obj_error(OBJ_SUB_STATE, EPERM);

        FS_LOG_DUMP_ERROR(
                "state transition failed: from=%u to=%u, err=%s (0x%x)",
                (unsigned int)objruntime_state(rt), (unsigned int)state,
                fs_error_str(err), err);
    ...
    FS_LOG_DUMP_INFO("exit: ok, new_state=%u", (unsigned int)state);
    return FS_OK;
}
...
fs_error_t objmgr_ref_get_locked(obj_runtime_t *rt)
{
    ...
    if (rt == NULL)
    {
        err = obj_error(OBJ_SUB_GET, EINVAL);
    ...
    {
        err = obj_error(OBJ_SUB_GET, EBUSY);

        FS_LOG_DUMP_ERROR("ref get failed: state=%u not ACTIVE, err=%s (0x%x)",
    ...
    FS_LOG_DUMP_INFO("exit: ok, refcnt=%d", (int)fs_atomic32_load(&rt->refcnt));
    return FS_OK;
}
...
fs_error_t objmgr_ref_put_locked(obj_runtime_t *rt)
{
    ...
    if (rt == NULL)
    {
        err = obj_error(OBJ_SUB_PUT, EINVAL);
    ...
    if (refcnt <= 0)
    {
        ...
        FS_LOG_DUMP_ERROR("invalid refcnt=%d, err=%s (0x%x)", refcnt,
                          fs_error_str(err), err);

        return err;
    }

    refcnt = fs_atomic32_dec(&rt->refcnt);

    FS_LOG_DUMP_INFO("ref released: refcnt=%d", refcnt);
    ...
    if (refcnt > 0)
    {
        FS_LOG_DUMP_INFO("exit: object still referenced");
    ...
    {
        FS_LOG_DUMP_INFO("exit: refcnt=0 but state=%u",
                         (unsigned int)objruntime_state(rt));

        return FS_OK;
    }

    FS_LOG_DUMP_INFO("final reclaim begin");
...
void objmgr_reclaim_locked(obj_runtime_t *rt)
{
    ...
    (void)objmgr_remove_locked(&rt->meta.key);
    ...
    FS_LOG_DUMP_INFO("object reclaimed: objectid=%llu gen=%u",
                     (unsigned long long)objectid, (unsigned int)gen);
