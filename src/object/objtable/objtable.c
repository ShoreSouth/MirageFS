...
static uint64_t objtable_node_hash(const fs_list_head_t *node)
{
    const objtable_entry_t *entry;

    entry = FS_CONTAINER_OF(node, objtable_entry_t, node);

    return objkey_hash(&entry->runtime->meta.key);
}

static uint64_t objtable_key_hash(const void *key)
{
    return objkey_hash(key);
}

static bool objtable_match(const fs_list_head_t *node, const void *key)
{
    const objtable_entry_t *entry;
...
static objtable_entry_t *objtable_find_entry(obj_table_t *table,
                                             const obj_key_t *key)
{
    fs_list_head_t *node;

    node = fs_hash_lookup(&table->table, key);
    ...
    return FS_CONTAINER_OF(node, objtable_entry_t, node);
}
...
fs_error_t objtable_init(obj_table_t *table, uint32_t bucket_nr)
{
    ...
    err = fs_hash_init(&table->table, bucket_nr, objtable_node_hash,
                       objtable_key_hash, objtable_match);
    if (fs_failed(err))
    {
...
void objtable_destroy(obj_table_t *table)
{
    ...
    {
        FS_LIST_FOR_EACH_SAFE(pos, next, &table->table.buckets[i])
        {
            entry = FS_CONTAINER_OF(pos, objtable_entry_t, node);

            fs_hash_remove(&table->table, &entry->node);
...
fs_error_t objtable_insert(obj_table_t *table, obj_runtime_t *runtime)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, runtime=%p", (void *)table,
                     (void *)runtime);
    ...
    key = objkey_make(runtime->meta.key.objectid, runtime->meta.key.gen);
    ...
    if (objtable_exists(table, &key))
    {
        ...
        FS_LOG_DUMP_ERROR("insert failed: object already exists, "
                          "key=(%lu,%u), err=%s (0x%x)",
                          (unsigned long)key.objectid, (unsigned int)key.gen,
                          fs_error_str(err), err);
        return err;
    }

    entry = calloc(1, sizeof(objtable_entry_t));
    ...
    err = fs_hash_insert(&table->table, &entry->node);
    if (fs_failed(err))
    {
...
fs_error_t objtable_remove(obj_table_t *table, const obj_key_t *key)
{
    ...
    entry = objtable_find_entry(table, key);

    if (entry == NULL)
    {
        err = obj_error(OBJ_SUB_REMOVE, FS_ERRNO_ENOENT);
        FS_LOG_DUMP_ERROR("remove failed: object not found, "
                          "key=(%lu,%u), err=%s (0x%x)",
                          (unsigned long)key->objectid, (unsigned int)key->gen,
                          fs_error_str(err), err);
        return err;
...
obj_runtime_t *objtable_lookup(obj_table_t *table, const obj_key_t *key)
{
    ...
    FS_LOG_DUMP_INFO("exit: found, runtime=%p", (void *)entry->runtime);
...
bool objtable_exists(obj_table_t *table, const obj_key_t *key)
{
    ...
    exists = (objtable_lookup(table, key) != NULL);
...
uint64_t objtable_count(const obj_table_t *table)
{
    ...
    {
        count = fs_hash_count(&table->table);
    }
