...
static fs_error_t fops_readdir_validate_ctx(const fops_readdir_ctx_t *ctx)
{
    ...
    if ((ctx == NULL) || (ctx->dir_fuid == NULL) || (ctx->entry_cap == 0U) ||
        (ctx->out_entry_nr == NULL) || (ctx->out_eof == NULL) ||
        (ctx->need_attr ? (ctx->plus_entries == NULL) : (ctx->entries == NULL)))
    {
...
static fs_error_t
fops_readdir_resolve_entry(const fops_readdir_resolve_ctx_t *ctx,
                           const char *name, fs_type_t type_hint,
                           fuid_t *out_fuid)
{
    fs_error_t err;
    ...
    err = fops_handle_from_lsa_checked(&handle, &lsa_handle, mount_id,
                                       ctx->dir_meta, ctx->sub);
    if (fs_failed(err))
    {
        return err;
    }

    return fops_fuid_from_handle(ctx->dir_fuid, &handle, type, out_fuid,
                                 ctx->sub);
}

static fs_error_t
fops_readdir_copy_plain(const fops_readdir_resolve_ctx_t *rctx,
                        const lsa_dirent_t *src, fops_dirent_t *dst)
{
    fs_error_t err;
...
static fs_error_t fops_readdir_copy_plus(const fops_readdir_resolve_ctx_t *rctx,
...
static fs_error_t fops_readdir_common(const fops_readdir_ctx_t *ctx)
{
    ...
    err = fops_open_object(ctx->dir_fuid, O_RDONLY | O_DIRECTORY | O_CLOEXEC,
                           &dir_meta, &dir_fd, ctx->sub);
    if (fs_failed(err))
    {
        goto out;
    }
    ...
    while (copied < ctx->entry_cap)
    {
        ...
        else
        {
            ...
            err = fops_readdir_copy_plain(&rctx, &entry, &ctx->entries[copied]);
        }
