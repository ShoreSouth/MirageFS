...
static fs_error_t fops_probe_existing_regular(int parent_fd, const char *name,
                                              fs_flags_t flags,
                                              bool *out_exists)
{
    fs_error_t err;
    ...
    *out_exists = false;
    ...
    err = lsa_lookup(parent_fd, name,
                     (flags & FS_FLAG_NOFOLLOW) | FS_FLAG_REGULAR, &fd);
    if (fs_failed(err))
    {
                            ...
                            fops_object_result_t *out)
{
    ...
    err = fops_validate_create_attr(
            attr,
            FOPS_CREATE_ATTR_MODE | FOPS_CREATE_ATTR_UID |
                    FOPS_CREATE_ATTR_GID | FOPS_CREATE_ATTR_SIZE,
            FS_OP_CREATE);
    if (fs_failed(err))
    {
        goto out;
    ...
    err = fops_open_object(parent_fuid, O_RDONLY | O_DIRECTORY | O_CLOEXEC,
                           &parent_meta, &parent_fd, FS_OP_CREATE);
    if (fs_failed(err))
    {
        goto out;
    }
    ...
    {
        err = fops_probe_existing_regular(parent_fd, name, flags,
                                          &existed_before);
        if (fs_failed(err))
        {
            goto out;
    ...
    err = fops_handle_from_lsa_checked(&handle, &lsa_handle, mount_id,
                                       parent_meta, FS_OP_CREATE);
    if (fs_failed(err))
    {
        goto rollback;
    }

    err = fops_fuid_from_handle(parent_fuid, &handle, FS_TYPE_REG, &out->fuid,
                                FS_OP_CREATE);
    if (fs_failed(err))
    {
        goto rollback;
