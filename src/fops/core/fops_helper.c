...
static fs_error_t fops_validate_known_flags(fs_flags_t flags, fs_flags_t known,
                                            fs_op_t sub)
{
    fs_error_t err;
...
static fs_error_t fops_validate_type_flag_pair(fs_flags_t flags, fs_op_t sub)
{
    fs_error_t err;
...
{
    if ((attr != NULL) && ((attr->valid_mask & FOPS_CREATE_ATTR_MODE) != 0U))
    {
                                     ...
                                     uint32_t supported_mask, fs_op_t sub)
{
    ...
    if ((attr->valid_mask & ~supported_mask) != 0U)
    {
        ...
        FS_LOG_DUMP_ERROR("create attr check failed: valid_mask=0x%x "
                          ...
                          attr->valid_mask, supported_mask, fs_error_str(err),
                          err);
        return err;
    }
                                  ...
                                  bool allow_size, fs_op_t sub)
{
    ...
    if (((attr->valid_mask & FOPS_CREATE_ATTR_SIZE) != 0U) && !allow_size)
    {
        err = fops_error(sub, EINVAL);
    ...
    {
        uid = ((attr->valid_mask & FOPS_CREATE_ATTR_UID) != 0U) ? attr->uid
                                                                : (uid_t)-1;
        gid = ((attr->valid_mask & FOPS_CREATE_ATTR_GID) != 0U) ? attr->gid
                                                                : (gid_t)-1;
                            ...
                            obj_meta_t **out_meta, int *out_fd, fs_op_t sub)
{
    ...
    *out_fd = -1;
    ...
    fd = -1;
    err = lsa_open_by_handle_id(meta->handle.mount_id, &lsa_handle, flags, &fd);
    if (fs_failed(err))
    {
        objmgr_release(meta);
        return err;
                                ...
                                int *out_fd, fs_op_t sub)
{
    ...
    return fops_open_object(fuid, O_RDONLY | O_DIRECTORY | O_CLOEXEC, out_meta,
                            out_fd, sub);
}
...
fuid_type_t fops_fuid_type_from_fs_type(fs_type_t type)
{
    ...
    {
    case FS_TYPE_REG:
        return FUID_TYPE_FILE;
    case FS_TYPE_DIR:
        return FUID_TYPE_DIR;
    case FS_TYPE_LNK:
        return FUID_TYPE_SYMLINK;
    case FS_TYPE_FIFO:
        return FUID_TYPE_FIFO;
    case FS_TYPE_SOCK:
        return FUID_TYPE_SOCK;
    case FS_TYPE_BLK:
        return FUID_TYPE_BLK;
    case FS_TYPE_CHR:
        return FUID_TYPE_CHR;
    default:
                            ...
                            GenId_t gen, fs_type_t type)
{
    ...
    fuid = fuid_make(parent_fuid->fsid, objectid, gen,
                                 ...
                                 fuid_t *out_fuid, fs_op_t sub)
{
    ...
    if (meta != NULL)
    {
        *out_fuid = fops_make_child_fuid(parent_fuid, meta->key.objectid,
                                         meta->key.gen, type);
        objmgr_release(meta);
        return FS_OK;
    }
    ...
    child_fuid = fops_make_child_fuid(parent_fuid, key.objectid, key.gen, type);
                                        ...
                                        fs_op_t sub)
{
    ...
    if (mount_id != parent_meta->handle.mount_id)
    {
        ...
        FS_LOG_DUMP_ERROR("cross mount object is unsupported: parent=%d, "
                          "child=%d, err=%s (0x%x)",
                          parent_meta->handle.mount_id, mount_id,
                          fs_error_str(err), err);
        return err;
    }
...
int fops_linux_open_flags(fs_flags_t flags)
{
    ...
    if (fs_flag_test(flags, FS_FLAG_WRITE) && fs_flag_test(flags, FS_FLAG_READ))
    {
        linux_flags = O_RDWR;
    }
...
fs_error_t fops_file_check(const fops_file_t *file, fs_op_t sub)
{
    ...
    if ((sub == FS_OP_READ) && fs_flag_test(file->flags, FS_FLAG_WRITE) &&
    ...
    if ((sub == FS_OP_WRITE) && !fs_flag_test(file->flags, FS_FLAG_WRITE))
    {
        err = fops_error(sub, EBADF);
