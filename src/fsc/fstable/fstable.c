...
static uint64_t fstable_name_hash(const char *name)
{
    uint64_t hash;
...
static uint64_t fstable_fsid_node_hash(const fs_list_head_t *node)
{
...
static uint64_t fstable_fsid_key_hash(const void *key)
{
...
static bool fstable_fsid_match(const fs_list_head_t *node, const void *key)
{
    const fsc_table_entry_t *entry;
    const fsc_fsid_t *fsid;
...
static uint64_t fstable_name_node_hash(const fs_list_head_t *node)
{
    const fsc_table_entry_t *entry;
...
static uint64_t fstable_name_key_hash(const void *key)
{
    return fstable_name_hash((const char *)key);
}

static bool fstable_name_match(const fs_list_head_t *node, const void *key)
{
    const fsc_table_entry_t *entry;
...
static fsc_table_entry_t *fstable_find_fsid_entry(fsc_table_t *table,
                                                  fsc_fsid_t fsid)
{
    fs_list_head_t *node;
...
static fsc_table_entry_t *fstable_find_name_entry(fsc_table_t *table,
                                                  const char *name)
{
    fs_list_head_t *node;
...
fs_error_t fstable_init(fsc_table_t *table, uint32_t bucket_nr)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, bucket_nr=%u", (void *)table, bucket_nr);
    ...
    err = fs_hash_init(&table->fsid_index, bucket_nr, fstable_fsid_node_hash,
                       fstable_fsid_key_hash, fstable_fsid_match);
    if (fs_failed(err))
    {
    ...
    err = fs_hash_init(&table->name_index, bucket_nr, fstable_name_node_hash,
                       fstable_name_key_hash, fstable_name_match);
    if (fs_failed(err))
    {
...
fs_error_t fstable_insert(fsc_table_t *table, fsc_namespace_t *ns)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, ns=%p", (void *)table, (void *)ns);
        ...
        fstable_exists_name(table, ns->name))
    {
        ...
        FS_LOG_DUMP_ERROR("insert failed: namespace already exists, "
                          ...
                          (unsigned long long)ns->fsid, ns->name,
                          fs_error_str(err), err);
        return err;
    }
                          ...
                          fsc_namespace_t **ns_out)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, fsid=%llu, ns_out=%p", (void *)table,
                     (unsigned long long)fsid, (void *)ns_out);
...
fsc_namespace_t *fstable_lookup_fsid(fsc_table_t *table, fsc_fsid_t fsid)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, fsid=%llu", (void *)table,
...
fsc_namespace_t *fstable_lookup_name(fsc_table_t *table, const char *name)
{
    ...
    FS_LOG_DUMP_INFO("enter: table=%p, name=%p", (void *)table,
                     (const void *)name);
