...
static fs_error_t fsc_sysroot_make_abs_path(
                const char *path,
                char *out,
                size_t out_size);

static fs_error_t fsc_sysroot_handle_from_lsa(
                obj_handle_t *out,
...
fs_error_t fsc_sysroot_init(const char *path)
{
    ...
    err = fsc_sysroot_make_abs_path(path,
                                    abs_path,
                                    sizeof(abs_path));
    if (fs_failed(err)) {
        fsc_sysroot_deinit();
    ...
    err = fsc_sysroot_handle_from_lsa(&g_sysroot.root_handle,
                                      &lsa_handle,
                                      mount_id);
    if (fs_failed(err)) {
        fsc_sysroot_deinit();
