...
static bool fsc_namespace_root_fuid_is_valid(fsc_fsid_t fsid,
                              ...
                              const obj_handle_t *root_handle)
{
    ...
    FS_LOG_DUMP_INFO("enter: ns=%p, fsid=%llu, name=%p", (void *)ns,
                     (unsigned long long)fsid, (const void *)name);

    if ((ns == NULL) || (root_fuid == NULL) || (root_handle == NULL))
    {
...
bool fsc_namespace_is_valid(const fsc_namespace_t *ns)
{
    ...
    valid = (ns != NULL) && fsid_is_valid(ns->fsid) &&
            fsc_namespace_name_is_valid(ns->name) &&
            fsc_namespace_root_fuid_is_valid(ns->fsid, &ns->root_fuid) &&
                                      ...
                                      fsc_namespace_state_t state)
{
    ...
    FS_LOG_DUMP_INFO("exit: ok, from=%u, to=%u", (unsigned int)old_state,
                     (unsigned int)state);
    return FS_OK;
}
