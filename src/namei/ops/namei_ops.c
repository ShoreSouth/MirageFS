...
static fs_error_t namei_dispatch_getattr(const fuid_t *fuid, fs_flags_t flags,
...
static fs_error_t namei_dispatch_readdir(const fuid_t *fuid, fs_flags_t flags,
...
static fs_error_t
namei_dispatch_readdirplus(const fuid_t *fuid, fs_flags_t flags,
...
static fs_error_t namei_dispatch_open(const fuid_t *fuid, fs_flags_t flags,
...
fs_error_t namei_lookup(const namei_ctx_t *ctx, const char *path,
                        fs_flags_t flags, fuid_t *out_fuid)
{
    ...
    if (namei_type_flags(flags) != FS_FLAG_NONE)
    {
        ...
        err = namei_dispatch_getattr(&result.fuid, namei_type_flags(flags),
                                     &attr);
        if (fs_failed(err))
        {
            return err;
...
fs_error_t namei_lookup_plus(const namei_ctx_t *ctx, const char *path,
...
fs_error_t namei_lookup_parent(const namei_ctx_t *ctx, const char *path,
                               fs_flags_t flags, namei_parent_result_t *out)
{
    fs_error_t err;
    ...
    if ((path == NULL) || (path[0] == '\0') ||
        (strlen(path) > FS_MAX_PATH_LEN) || (strcmp(path, "/") == 0) ||
    ...
    len = strlen(name);
    if ((len == 0U) || (len > FS_MAX_NAME_LEN) || (strchr(name, '/') != NULL) ||
...
fs_error_t namei_create(const namei_ctx_t *ctx, const char *path,
                        ...
                        fops_object_result_t *out)
{
    ...
    return namei_dispatch_create_plus(&parent.parent_fuid, parent.name, attr,
                                      flags, out);
}

fs_error_t namei_mkdir(const namei_ctx_t *ctx, const char *path,
                       const fops_create_attr_t *attr, fs_flags_t flags,
                       fops_object_result_t *out)
{
    fs_error_t err;
    ...
    return namei_dispatch_mkdir_plus(&parent.parent_fuid, parent.name, attr,
                                     flags, out);
}

fs_error_t namei_symlink(const namei_ctx_t *ctx, const char *target,
                         const char *linkpath, fs_flags_t flags,
                         fops_object_result_t *out)
{
    fs_error_t err;
    namei_parent_result_t parent;
...
fs_error_t namei_readlink(const namei_ctx_t *ctx, const char *path,
...
fs_error_t namei_readdir(const namei_ctx_t *ctx, const char *path,
                         fs_flags_t flags, const namei_readdir_args_t *args)
{
    fs_error_t err;
    fuid_t fuid;
...
fs_error_t namei_readdirplus(const namei_ctx_t *ctx, const char *path,
                             fs_flags_t flags,
...
fs_error_t namei_unlink(const namei_ctx_t *ctx, const char *path,
                        fs_flags_t flags)
{
    fs_error_t err;
    namei_parent_result_t parent;
...
fs_error_t namei_rmdir(const namei_ctx_t *ctx, const char *path,
                       fs_flags_t flags)
{
    fs_error_t err;
    namei_parent_result_t parent;
...
fs_error_t namei_rename(const namei_ctx_t *ctx, const char *old_path,
                        const char *new_path, fs_flags_t flags)
{
    fs_error_t err;
    ...
    return namei_dispatch_rename(&old_parent.parent_fuid, old_parent.name,
                                 &new_parent.parent_fuid, new_parent.name,
                                 flags);
}
