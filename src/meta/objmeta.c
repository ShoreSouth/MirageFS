...
static int32_t objmeta_handle_valid(uint16_t handle_bytes)
{
    if (handle_bytes == 0) {
        return 0;
    }
                    ...
                    const uint8_t *file_handle)
{
    ...
    if (file_handle == NULL) {
        FS_LOG_DUMP_ERROR("file_handle is NULL");
        return -1;
    }

    if (!objmeta_handle_valid(handle_bytes)) {
        FS_LOG_DUMP_ERROR("invalid handle_bytes=%u",
                  handle_bytes);
        return -1;
    }

    memset(meta, 0, sizeof(ObjMeta_t));
...
int32_t objmeta_is_valid(const ObjMeta_t *meta)
{
    ...
    if (!objmeta_handle_valid(meta->handle_bytes)) {
        return 0;
    }
...
void objmeta_dump(const ObjMeta_t *meta)
{
    ...
    for (i = 0; i < meta->handle_bytes; i++) {

        offset += snprintf(handle_buf + offset,
    ...
    FS_LOG_DUMP_INFO("========== ObjMeta ==========");
    FS_LOG_DUMP_INFO("objectid     : %lu", meta->objectid);
    FS_LOG_DUMP_INFO("mount_id     : %d", meta->mount_id);
    FS_LOG_DUMP_INFO("handle_type  : %u", meta->handle_type);
    FS_LOG_DUMP_INFO("handle_bytes : %u", meta->handle_bytes);
    FS_LOG_DUMP_INFO("file_handle  : %s", handle_buf);
    FS_LOG_DUMP_INFO("=============================");
}