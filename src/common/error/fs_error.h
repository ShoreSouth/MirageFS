...
#define FS_ERR(sev, module, sub, err)                                          \
    ((fs_error_t)((((uint32_t)(sev)&FS_ERR_SEV_MASK) << FS_ERR_SEV_SHIFT) |    \
                  (((uint32_t)(module)&FS_ERR_MODULE_MASK)                     \
                   << FS_ERR_MODULE_SHIFT) |                                   \
...
static inline fs_err_severity_t fs_err_severity(fs_error_t err)
{
    return (fs_err_severity_t)((err >> FS_ERR_SEV_SHIFT) & FS_ERR_SEV_MASK);
}

static inline fs_module_t fs_err_module(fs_error_t err)
{
    return (fs_module_t)((err >> FS_ERR_MODULE_SHIFT) & FS_ERR_MODULE_MASK);
}

static inline uint32_t fs_err_sub(fs_error_t err)
{
    return (uint32_t)((err >> FS_ERR_SUB_SHIFT) & FS_ERR_SUB_MASK);
}

static inline fs_errno_t fs_err_errno(fs_error_t err)
{
    return (fs_errno_t)(err & FS_ERRNO_MASK);
}
...
static inline bool fs_succeeded(fs_error_t err)
{
    return err == FS_OK;
}

static inline bool fs_failed(fs_error_t err)
{
    return err != FS_OK;
}
