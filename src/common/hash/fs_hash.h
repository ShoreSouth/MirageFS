 ...
 * 插入节点时使用。
 */
typedef uint64_t (*fs_hash_node_hash_fn)(const fs_list_head_t *node);
 ...
 * 查找节点时使用。
 */
typedef uint64_t (*fs_hash_key_hash_fn)(const void *key);
 ...
 * key 为业务自定义查询条件。
 */
typedef bool (*fs_hash_match_fn)(const fs_list_head_t *node, const void *key);
...
static inline uint64_t fs_hash_count(const fs_hash_t *hash)
{
