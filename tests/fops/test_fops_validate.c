...
const test_case_t FOPS_VALIDATE_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1),
                  UT_CASE_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1,
                             0x001),
                  test_fops_validate_name_accepts_dot_only_when_allowed,
                  "FOPS 名称校验", "普通名、点名、带 slash 名称和超长名称",
                  "dot 仅在允许时通过，非法名称被拒绝"),
        TEST_CASE(UT_LIST_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1),
                  UT_CASE_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1,
                             0x002),
                  test_fops_validate_flags_rejects_unknown_and_conflict_bits,
                  "FOPS flag 校验", "注入互斥 flag、不支持 flag 和非法 op",
                  "返回 FOPS/EINVAL"),
        TEST_CASE(UT_LIST_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1),
                  UT_CASE_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1,
                             0x003),
        ...
        TEST_CASE(UT_LIST_NO(UT_MOD_FOPS, TEST_FOPS_COMPONENT_VALIDATE, 0x1),
                  ...
                  "FOPS 类型约束", "目录/普通文件 flag 与实际类型不匹配",
