...
const test_case_t OBJECT_OBJMETA_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x1),
                  UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x1,
                             0x001),
                  test_objmeta_handle_lsa_round_trip, "ObjMeta handle 转换",
                  "构造带 mount/type/len/data 的 LSA handle，并与 Object "
                  "handle 往返转换",
                  "mount/type/len/data 全部保持一致，没有截断或字段错位"),
        TEST_CASE(
                UT_LIST_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x1),
                UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x1,
                           0x002),
                test_objmeta_init_equal_and_reset, "ObjMeta 初始化与比较",
                "用合法 FUID/handle 初始化，复制后篡改 handle data，再执行 "
                "deinit",
        ...
        TEST_CASE(
                ...
                UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x1,
                           0x003),
                test_objmeta_rejects_invalid_inputs, "ObjMeta 参数校验",
                "转换 LSA handle 时注入 NULL 输出参数和超过上限的 handle 长度",
                "返回 OBJECT 模块 EINVAL/EOVERFLOW，错误归属不被包装丢失"),
        TEST_CASE(UT_LIST_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x2),
                  UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJMETA, 0x2,
                             0x001),
...
const size_t OBJECT_OBJMETA_CASE_COUNT =
        sizeof(OBJECT_OBJMETA_CASES) / sizeof(OBJECT_OBJMETA_CASES[0]);
