...
const test_case_t OBJECT_OBJRUNTIME_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJRUNTIME,
                             0x1),
                  UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJRUNTIME,
                             0x1, 0x001),
                  test_objruntime_state_reads_stored_state,
                  "ObjRuntime 状态读取", "直接设置 ACTIVE/DELETING 状态",
                  "getter 返回当前保存状态"),
        TEST_CASE(UT_LIST_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJRUNTIME,
                             0x2),
                  UT_CASE_NO(UT_MOD_OBJECT, TEST_OBJECT_COMPONENT_OBJRUNTIME,
                  ...
                  "ObjRuntime 状态边界读取", "直接设置 INIT/INVALID 状态",
                  "getter 返回 runtime 当前保存状态"),
};

const size_t OBJECT_OBJRUNTIME_CASE_COUNT =
        sizeof(OBJECT_OBJRUNTIME_CASES) / sizeof(OBJECT_OBJRUNTIME_CASES[0]);
