...
int test_run_suite(const char *suite, const test_case_t *cases,
                   size_t nr_cases);
...
#define UT_NO_MAKE(module, component, list, item)                              \
    (((((ut_no_t)(module)) & 0xffU) << 24) |                                   \
     ...
     ((((ut_no_t)(list)) & 0x0fU) << 12) | (((ut_no_t)(item)) & 0x0fffU))
...
#define TEST_ASSERT_TRUE(expr)                                                 \
    ...
    {                                                                          \
        if (!(expr))                                                           \
            return test_fail_at(__FILE__, __LINE__);                           \
    } while (0)
...
#define TEST_ASSERT_EQ_INT(actual, expected)                                   \
    ...
    {                                                                          \
        ...
        {                                                                      \
            return test_fail_eq_int(__FILE__, __LINE__, test_actual__,         \
                                    test_expected__);                          \
        }                                                                      \
    } while (0)
