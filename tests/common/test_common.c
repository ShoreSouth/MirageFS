...
int main(void)
{
    const test_case_t cases[] = {
        ...
        TEST_CASE(test_module_and_op_helpers_handle_valid_and_invalid_values,
                  "模块和操作名 helper",
                  "传入合法枚举、NONE/MAX 边界值",
                  "合法值返回名称，非法值返回 UNKNOWN 或 false"),
        TEST_CASE(test_flag_helper_detects_set_and_missing_bits,
                  "flag 位检测",
        ...
        TEST_CASE(test_path_normalize_removes_dotdot,
                  ...
                  "输入包含重复斜杠、点和点点片段",
                  "返回折叠后的规范路径"),
        TEST_CASE(test_path_normalize_collapses_leading_dotdot,
