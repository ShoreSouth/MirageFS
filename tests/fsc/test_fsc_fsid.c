...
const test_case_t FSC_FSID_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1),
                  UT_CASE_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1, 0x001),
                  test_fsid_alloc_free_and_stale_guard, "FSID 分配释放",
                  "分配后释放，并重复释放旧 FSID",
                  "第一次成功，重复释放被识别为非法/stale"),
        TEST_CASE(UT_LIST_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1),
                  UT_CASE_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1, 0x002),
                  test_fsid_rejects_null_output, "FSID 参数校验",
                  "fsid_alloc 传入 NULL 输出参数", "返回 FSC 模块 EINVAL"),
        TEST_CASE(UT_LIST_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1),
                  UT_CASE_NO(UT_MOD_FSC, TEST_FSC_COMPONENT_FSID, 0x1, 0x003),
...
const size_t FSC_FSID_CASE_COUNT =
        sizeof(FSC_FSID_CASES) / sizeof(FSC_FSID_CASES[0]);
