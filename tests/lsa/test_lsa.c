...
{
    int written = snprintf(path, size, "/tmp/miragefs-lsa-test-%ld-XXXXXX", (long)getpid());
...
static int test_lsa_create_rejects_conflicting_flags(void)
{
    ...
    err = lsa_create(dirfd, "bad.txt", FS_FLAG_REPLACE | FS_FLAG_EXCLUSIVE, 0644, &fd);
    TEST_ASSERT_TRUE(fs_failed(err));
    TEST_ASSERT_EQ_INT(fs_err_module(err), FS_MODULE_LSA);
...
int main(void)
{
    const test_case_t cases[] = {
        TEST_CASE(test_lsa_create_write_read_round_trip,
                  "在真实临时目录中创建文件并完成写读回环",
                  "使用真实 tmpdir 后端，不强制 syscall 失败",
                  "create/write/read 均返回 FS_OK，且字节内容一致"),
        TEST_CASE(test_lsa_lookup_missing_returns_enoent,
                  "在已存在目录下 lookup 一个不存在的条目",
                  "Linux openat 返回 ENOENT",
                  "LSA 将失败映射为 FS_MODULE_LSA/ENOENT"),
        TEST_CASE(test_lsa_create_rejects_conflicting_flags,
