...
static int test_lsa_lookup_flag_matrix(void)
{
    ...
    fd = -1;
    TEST_ASSERT_EQ_INT(lsa_mkdir(dirfd, "dir", FS_FLAG_DIRECTORY, 0755), FS_OK);
    ...
    err = lsa_lookup(dirfd, "plain.txt", FS_FLAG_DIRECTORY | FS_FLAG_REGULAR,
                     &fd);
...
const test_case_t LSA_LOOKUP_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_LOOKUP, 0x1),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_LOOKUP, 0x1, 0x001),
...
const size_t LSA_LOOKUP_CASE_COUNT =
        sizeof(LSA_LOOKUP_CASES) / sizeof(LSA_LOOKUP_CASES[0]);
