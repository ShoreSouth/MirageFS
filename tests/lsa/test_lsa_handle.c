...
static int test_lsa_handle_capability_paths(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_open_by_handle_at(-1,
                                                              NULL,
                                                              O_RDONLY,
                                                              &opened),
                                        FS_ERRNO_EINVAL), 0);
...
static int test_lsa_bootstrap_root_and_init(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_bootstrap_root(NULL,
                                                           &handle,
                                                           &mount_id),
                                        FS_ERRNO_EINVAL), 0);
    test_lsa_cleanup_tmpdir(dir_path);
...
const size_t LSA_HANDLE_CASE_COUNT = sizeof(LSA_HANDLE_CASES) / sizeof(LSA_HANDLE_CASES[0]);
