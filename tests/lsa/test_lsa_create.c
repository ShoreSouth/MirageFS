...
static int test_lsa_create_flag_matrix(void)
{
    ...
    err = lsa_create(dirfd, "matrix.txt", FS_FLAG_EXCLUSIVE, 0644, &fd);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(err, FS_ERRNO_EEXIST), 0);

    err = lsa_create(dirfd, "matrix.txt", FS_FLAG_REPLACE | FS_FLAG_TRUNCATE,
                     0644, &fd);
    TEST_ASSERT_EQ_INT(err, FS_OK);
    TEST_ASSERT_EQ_INT(lsa_close(fd), FS_OK);
    fd = -1;
    ...
    err = lsa_create(dirfd, "bad.txt", FS_FLAG_REPLACE | FS_FLAG_EXCLUSIVE,
                     0644, &fd);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(err, FS_ERRNO_EINVAL), 0);
...
const test_case_t LSA_CREATE_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_CREATE, 0x1),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_CREATE, 0x1, 0x001),
...
const size_t LSA_CREATE_CASE_COUNT =
        sizeof(LSA_CREATE_CASES) / sizeof(LSA_CREATE_CASES[0]);
