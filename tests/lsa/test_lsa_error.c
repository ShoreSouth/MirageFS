...
static int test_lsa_invalid_argument_matrix(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_read(-1, NULL, 1, &actual),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_read_full(-1, NULL, 1, &actual),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_write(-1, NULL, 1, &actual),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_write_full(-1, NULL, 1, &actual),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_pread(-1, NULL, 1, 0, &actual),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_pwrite(-1, NULL, 1, 0, &actual),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
    ...
    TEST_ASSERT_EQ_INT(
            ...
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_statfs(-1, NULL), FS_ERRNO_EINVAL), 0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(
                               lsa_readlink(AT_FDCWD, NULL, &byte, 1, &actual),
                               FS_ERRNO_EINVAL),
                       0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_listxattr(-1, NULL, 0, NULL),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_getxattr(-1, NULL, &byte, 1, &actual),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_setxattr(-1, NULL, &byte, 1, 0),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_removexattr(-1, NULL), FS_ERRNO_EINVAL),
            0);
...
static int test_lsa_syscall_failure_matrix(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_close(-1), FS_ERRNO_EBADF), 0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_read(-1, &byte, 1, &actual),
                                             FS_ERRNO_EBADF),
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_read_full(-1, &byte, 1, &actual),
                                  FS_ERRNO_EBADF),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_write(-1, &byte, 1, &actual),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_write_full(-1, &byte, 1, &actual),
                                  FS_ERRNO_EBADF),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_pread(-1, &byte, 1, 0, &actual),
                                  FS_ERRNO_EBADF),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_pwrite(-1, &byte, 1, 0, &actual),
                                  FS_ERRNO_EBADF),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_failed(lsa_lseek(-1, 0, SEEK_SET, &offset)), 0);
    ...
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_fchown(-1, (uid_t)-1, (gid_t)-1),
                                  FS_ERRNO_EBADF),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_failed(lsa_faccess(-1, R_OK)), 0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_statfs(-1, &fsst), FS_ERRNO_EBADF), 0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_syncfs(-1), FS_ERRNO_EBADF),
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_failed(lsa_listxattr(-1, &byte, 1, &actual)), 0);
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_dir_iter_next(iter, &entry),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_failed(lsa_dir_iter_seek(iter,
                                                     (lsa_dir_cookie_t){
                                                             0,
                                                     })),
            0);
...
const test_case_t LSA_ERROR_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_ERROR, 0x1),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_ERROR, 0x1, 0x001),
        ...
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_ERROR, 0x2),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_ERROR, 0x2, 0x001),
...
const size_t LSA_ERROR_CASE_COUNT =
        sizeof(LSA_ERROR_CASES) / sizeof(LSA_ERROR_CASES[0]);
