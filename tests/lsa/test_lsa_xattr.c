...
static int test_lsa_xattr_flag_matrix(void)
{
    ...
    err = lsa_setxattr(fd, "user.miragefs.test", "one", 3, FS_FLAG_EXCLUSIVE);
    if (fs_failed(err))
    {
    ...
    TEST_ASSERT_EQ_INT(lsa_getxattr(fd, "user.miragefs.test", value,
                                    sizeof(value), &actual),
                       FS_OK);
    TEST_ASSERT_EQ_INT(actual, 3);
    TEST_ASSERT_STR_EQ(value, "one");

    err = lsa_setxattr(fd, "user.miragefs.test", "two", 3, FS_FLAG_EXCLUSIVE);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(err, FS_ERRNO_EEXIST), 0);
    TEST_ASSERT_EQ_INT(
            lsa_setxattr(fd, "user.miragefs.test", "two", 3, FS_FLAG_REPLACE),
            FS_OK);
    ...
    err = lsa_getxattr(fd, "user.miragefs.test", value, sizeof(value), &actual);
    TEST_ASSERT_TRUE(fs_failed(err));
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(
                    lsa_setxattr(fd, "user.bad", "x", 1,
...
const test_case_t LSA_XATTR_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_XATTR, 0x1),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_XATTR, 0x1, 0x001),
...
const size_t LSA_XATTR_CASE_COUNT =
        sizeof(LSA_XATTR_CASES) / sizeof(LSA_XATTR_CASES[0]);
