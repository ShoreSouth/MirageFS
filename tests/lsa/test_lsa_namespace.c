...
static int test_lsa_namespace_round_trip(void)
{
    ...
    TEST_ASSERT_EQ_INT(lsa_mkdir(dirfd, "sub", FS_FLAG_DIRECTORY, 0755), FS_OK);
    ...
    TEST_ASSERT_EQ_INT(lsa_mknod(dirfd, "pipe", FS_TYPE_FIFO, 0644, NULL),
                       FS_OK);
    TEST_ASSERT_EQ_INT(lsa_unlink(dirfd, "pipe", 0), FS_OK);
    TEST_ASSERT_EQ_INT(lsa_unlink(dirfd, "sym", FS_FLAG_NOFOLLOW), FS_OK);
    TEST_ASSERT_EQ_INT(lsa_unlink(dirfd, "moved.txt", FS_FLAG_REGULAR), FS_OK);
    TEST_ASSERT_EQ_INT(lsa_unlink(dirfd, "source.txt", FS_FLAG_REGULAR), FS_OK);
...
static int test_lsa_namespace_error_matrix(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_mkdir(dirfd, NULL, 0, 0755),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_mkdir(dirfd, "bad", 0x80000000U, 0755),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_unlink(dirfd, NULL, 0), FS_ERRNO_EINVAL),
            0);
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_rmdir(dirfd, "afile", 0),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_rename(dirfd, NULL, dirfd, "x", 0),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_link(dirfd, NULL, dirfd, "x", 0),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_symlink(NULL, dirfd, "s", 0),
                                             FS_ERRNO_EINVAL),
                       0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(
                               lsa_mknod(dirfd, NULL, FS_TYPE_FIFO, 0644, NULL),
                               FS_ERRNO_EINVAL),
                       0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(
                               lsa_mknod(dirfd, "blk", FS_TYPE_BLK, 0644, NULL),
                               FS_ERRNO_EINVAL),
                       0);
...
static int test_lsa_namespace_failure_edges(void)
{
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(lsa_lookup(dirfd, "src", 0, NULL),
                       ...
                       0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_create(dirfd, NULL, 0, 0644, &fd),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_create(dirfd, "bad", 0, 0644, NULL),
                                  FS_ERRNO_EINVAL),
            0);
    err = lsa_create(dirfd, "append.txt",
                     FS_FLAG_APPEND | FS_FLAG_SYNC | FS_FLAG_NOFOLLOW, 0644,
                     &fd);
    ...
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_rmdir(dirfd, NULL, 0), FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_errno(lsa_rmdir(dirfd, "missing", 0x80000000U),
                                  FS_ERRNO_EINVAL),
            0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_failed(
                               lsa_rename(dirfd, "missing", dirfd, "dst", 0)),
                       0);
    TEST_ASSERT_EQ_INT(lsa_symlink("src", dirfd, "sym", 0), FS_OK);
    TEST_ASSERT_EQ_INT(
            test_lsa_assert_failed(lsa_symlink("src", dirfd, "sym", 0)), 0);
    TEST_ASSERT_EQ_INT(test_lsa_assert_failed(
                               lsa_readlink(dirfd, "missing-link", dir_path,
                                            sizeof(dir_path), &(size_t){0})),
                       0);
    ...
    TEST_ASSERT_EQ_INT(test_lsa_assert_errno(
                               lsa_mknod(dirfd, "chr", FS_TYPE_CHR, 0644, NULL),
                               FS_ERRNO_EINVAL),
                       0);
...
const test_case_t LSA_NAMESPACE_CASES[] = {
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x1),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x1,
                             0x001),
                  test_lsa_namespace_round_trip, "namespace 操作真实后端回环",
                  "执行 mkdir/link/rename/symlink/readlink/mknod/unlink/rmdir",
                  "命名空间变更可观察且最终临时目录可完整清理"),
        TEST_CASE(UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x2),
                  UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x2,
                             0x001),
        ...
        TEST_CASE(
                UT_LIST_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x3),
                UT_CASE_NO(UT_MOD_LSA, TEST_LSA_COMPONENT_NAMESPACE, 0x3,
                           0x001),
...
const size_t LSA_NAMESPACE_CASE_COUNT =
        sizeof(LSA_NAMESPACE_CASES) / sizeof(LSA_NAMESPACE_CASES[0]);
